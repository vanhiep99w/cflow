...
Each agent claims its task atomically before starting work:
...
After successful integration of each subtask:

```bash
bd update <task-id> --done
```
...
- If conflict (task already claimed by another agent): pick next from `bd ready`
- No concurrent edits: one subtask per agent (enforced by --claim)
- Progress reporting: subagent completes → `bd update --done` → parent sees via `bd ready`
